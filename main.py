...
class Post(db.Model):
    ...
    # def link_delete(self, a):
    #     key = db.Key.from_path('Post', int(a), parent=blog_key())
    #     post = db.get(key)
    #     db.delete(key)
    #     return "/"
...
#     def get(self, post_id):
#         userposted = self.request.get('userposted')
#         useractual = self.request.get('useractual')

#         if not self.user:
#             self.redirect('/login')
#         elif useractual == userposted:
#             key = db.Key.from_path('Post', int(post_id), parent=blog_key())
#             post = db.get(key)
#             db.delete(key)
#             posts  = Post.all().order('-created')
#             self.render('front.html', posts = posts)
#         else:
#             error = "Only the user who posted this blog can delete it!"
#             key = db.Key.from_path('Post', int(post_id), parent=blog_key())
#             post = db.get(key)
#             self.render('/blog/%s' % str(post.key().id()))
